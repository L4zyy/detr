...
class CTransformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 ...
                 return_intermediate_dec=False, merge_layer=False):
        super().__init__()

        encoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before)
        encoder_norm = nn.LayerNorm(d_model) if normalize_before else None
        self.encoder = CTransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)

        decoder_layer = TransformerDecoderLayer(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before)
        decoder_norm = nn.LayerNorm(d_model)
        self.decoder = CTransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        return hs.transpose(1, 2), memory[-1].permute(1, 2, 0).view(bs, c, h, w)
...
class CTransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        for layer in self.layers:
            interm = layer(interm, src_mask=mask,
...
def build_ctransformer(args):
    if args.merge:
        merge_layer = MergeLayer(args.hidden_dim, args.dim_feedforward, args.dropout)
    else:
        merge_layer = False
    return CTransformer(
